...
+ DRY is a principle of software development which
states that "Every piece of knowledge must have a single, unambiguous, authoritative
representation within a system." By not writing the same information over and over
again, our code is more maintainable, more extensible, and less buggy.
+ Rails has opinions about the best way to do many
...
**Info:** The examples below use `$` to represent your terminal prompt in a UNIX-like OS,
...
**Info:** A number of tools exist to help you quickly install Ruby and Ruby
...
**Info:** You can see all of the command line options that the Rails application
...
**Info:** Compiling CoffeeScript and JavaScript asset compression requires you
...
![](images/getting_started/rails_welcome.png)

**Info:** To stop the web server, hit Ctrl+C in the terminal window where it's
...
The "Welcome aboard" page is the  for a new Rails application: it
makes sure that you have your software configured correctly enough to serve a
page. You can also click on the  link to
see a summary of your application's environment.

##  4.2 Say "Hello", Rails

To get Rails saying "Hello", you need to create at minimum a  and a
.
...
different . Each action's purpose is to collect information to provide
it to a view.

A view's purpose is to display this information in a human readable format. An
important distinction to make is that it is the , not the view,
...
navigate to the root URL of our site, [http://localhost:3000](http://localhost:3000). At the moment,
...
This is your application's  which holds entries in a special DSL
...
tells Rails to map requests to [http://localhost:3000/welcome/index](http://localhost:3000/welcome/index) to the
welcome controller's index action. This was created earlier when you ran the
controller generator (`rails generate controller welcome index`).

Launch the web server again if you stopped it to generate the controller (`rails
server`) and navigate to [http://localhost:3000](http://localhost:3000) in your browser. You'll see the
"Hello, Rails!" message you put into `app/views/welcome/index.html.erb`,
indicating that this new route is indeed going to `WelcomeController`'s `index`
action and is rendering the view correctly.

**Info:** For more information about routing, refer to [Rails Routing from the Outside In](routing.html).
...
In the Blog application, you will now create a new . A resource is the
term used for a collection of similar objects, such as articles, people or
animals.
You can create, read, update and destroy items for a resource and these
operations are referred to as  operations.

Rails provides a `resources` method which can be used to declare a standard REST
resource. You need to add the  to the
...
![](images/getting_started/new_article.png)
...
Navigate to [http://localhost:3000/articles/new](http://localhost:3000/articles/new) and you'll see a routing
error:

![](images/getting_started/routing_error_no_controller.png)
...
If you refresh [http://localhost:3000/articles/new](http://localhost:3000/articles/new) now, you'll get a new error:

![](images/getting_started/unknown_action_new_for_articles.png)
...
[http://localhost:3000/articles/new](http://localhost:3000/articles/new) you'll see another error:
...
us what  could be used to render our template. `:erb` is most
...
first extension is the  of the template, and the second extension is the
 that will be used. Rails is attempting to find a template called
...
When you refresh [http://localhost:3000/articles/new](http://localhost:3000/articles/new) you'll now see that the
...
To create a form within this template, you will use a . The primary form builder for Rails is provided by a helper
...
. These parameters can then be referenced inside the controller
...
**Info:** Ensure you have a firm grasp of the `params` method, as you'll use it fairly regularly. Let's consider an example URL: . In this URL, `params[:username]` would equal "dhh" and `params[:email]` would equal "[dhh@email.com](mailto:dhh@email.com)".
...
with a  attribute of type string, and a  attribute
...
**Info:** Active Record is smart enough to automatically map column names to model
...
As we've just seen, `rails generate model` created a  file
...
**Info:** For more information about migrations, refer to [Rails Database Migrations](migrations.html).
...
**Info:** You might be wondering why the `A` in `Article.new` is capitalized above, whereas most other references to articles in this guide have used lowercase. In this context, we are referring to the class named `Article` that is defined in `\models\article.rb`. Class names in Ruby must begin with a capital letter.

**Info:** As we'll see later, `@article.save` returns a boolean indicating whether
the article was saved or not.

If you now go to [http://localhost:3000/articles/new](http://localhost:3000/articles/new) you'll  be able
...
**Info:** For more information, refer to the reference above and
...
Visit [http://localhost:3000/articles/new](http://localhost:3000/articles/new) and give it a try!

![](images/getting_started/show_action_for_articles.png)
...
Now if you go to [http://localhost:3000/articles](http://localhost:3000/articles) you will see a list of all the
...
**Info:** If you want to link to an action in the same controller, you don't need to
specify the `:controller` option, as Rails will use the current controller by
default.

**Info:** In development mode (which is what you're working in by default), Rails
...
If you reload
[http://localhost:3000/articles/new](http://localhost:3000/articles/new) and
...
**Info:** Rails automatically wraps fields that contain an error with a div
...
![](images/getting_started/form_with_errors.png)
...
via the `PATCH` HTTP method which is the HTTP method you're expected to use to
 resources according to the REST protocol.

The first parameter of `form_for` can be an object, say, `@article` which would
cause the helper to fill in the form with the fields of the object. Passing in a
symbol (`:article`) with the same name as the instance variable (`@article`)
also automagically leads to the same behavior. This is what is happening here.
More details can be found in [form_for documentation](http://api.rubyonrails.org/classes/ActionView/Helpers/FormHelper.html#method-i-form_for).
...
**Info:** You don't need to pass all attributes to `update`. For
...
![](images/getting_started/index_action_with_edit_link.png)
...
**Info:** You can read more about partials in the
...
to stand in for either of the other forms is that `@article` is a 
corresponding to a full set of RESTful routes, and Rails is able to infer
which URI and method to use.
For more information about this use of `form_for`, see [Resource-oriented style](http://api.rubyonrails.org/classes/ActionView/Helpers/FormHelper.html#method-i-form_for-label-Resource-oriented+style).
...
![](images/getting_started/confirm_dialog.png)

Congratulations, you can now create, show, list, update and destroy
articles.

**Info:** In general, Rails encourages using resources objects instead of
...
is the line `belongs_to :article`, which sets up an Active Record .
...
**Info:** For more information on Active Record associations, see the [Active Record
...
This creates `comments` as a  within `articles`. This is
another part of capturing the hierarchical relationship that exists between
articles and comments.

**Info:** For more information on routing, see the [Rails Routing](routing.html)
guide.
...
![](images/getting_started/article_with_comments.png)
...
![](images/getting_started/challenge.png)
...
**Info:** To be able to generate the Rails Guides locally with the `doc:guides` rake
