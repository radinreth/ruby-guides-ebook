...
This method is only useful  validations have been run, because it only
...
The default error message for this helper is .
...
The default error message for `validates_associated` is . Note
...
Alternatively, you can require that the specified attribute does  match the regular expression by using the `:without` option.
...
value. The default error message for this option is .
...
default error message for this option is .
...
supplied value. The default error message for this option is .
...
Note that the validator will be initialized  for the whole application
...
option when you want to specify when the validation  happen. If you
want to specify when the validation  happen, then you may use the
`:unless` option.
