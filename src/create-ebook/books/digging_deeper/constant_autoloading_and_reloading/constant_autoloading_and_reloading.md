---
title: Constant Autoloading and Reloading
...
The  at any given place is the collection of enclosing nested class and
...
, it has nothing to do with the constants used to access them, and is
...
That is why we talk about  autoloading, Rails has the ability to
...
At any given place in the code, let's define  to be the first element of
...
Rails autoloading , but its starting point is
...
its resolution next. Let's call  to that qualifying class or module
object, that is, `Billing` in the example above. The algorithm for qualified
constants goes like this:

As you see, this algorithm is simpler than the one for relative constants. In
particular, the nesting plays no role here, and modules are not special-cased,
if neither they nor their ancestors have the constants, `Object` is 
checked.
...
Given a string with a constant path we define its  to be the
...
missing constant, autoloading is  triggered.
...
this guide for the definition of , and [Resolution Algorithm for Qualified
...
the boot process. But constant autoloading in Rails is  implemented with
`Module#autoload`.
...
`Rectangle.all` should now return rectangles  squares:
...
That is not a bug, the query includes all  descendants of `Rectangle`.
...
Only the leaves that are  need to be loaded this
...
`User` constant is known at that point, in particular in a  call to
`user`:
