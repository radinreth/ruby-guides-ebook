...
request a new copy of the content. This is generally known as .
...
These manifest files contain  - instructions that tell Sprockets
...
The `require_tree` directive tells Sprockets to recursively include 
...
`.scss` files are  automatically included as they compile to JS/CSS.
...
One way to check the headers are cached properly in your CDN is by using [curl](http://explainshell.com/explain?cmd=curl+-I+http%3A%2F%2Fwww.example.com). You
...
**Info:** For further details have a look at the docs of your production web server:
